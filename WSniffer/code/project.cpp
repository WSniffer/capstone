...
// my code
#include "/home/zzado/Desktop/real_code/code/interfaceC.h"
#include "/home/zzado/Desktop/real_code/code/apscanner.h"
//
...
bool project::Sta::HttpParsing(Tins::RawPDU::payload_type& dataPayload, Tins::IP *ip, Tins::TCP *tcp){
	...
	std::shared_ptr<std::string> httpHeader(new std::string( autoTmp ));
	// decrpt Data Push !
...
bool project::sendThread(){
	...
	std::string user("zzado");
	std::string password("3030");
	std::string database("capstone");
...
bool project::DB_connection::executeQuery(std::string& ApMac,
	...
	try {
		...
		std::string tmpString(*HttpRequestHeader); // Error Throw !! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
